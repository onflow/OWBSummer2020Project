// BaloonToken.cdc
//
// The BaloonToken contract is a sample implementation of a fungible token on Flow.
//
...
pub contract BaloonToken {
    ...
    pub fun testMintTokens(amount: UFix64, recipient: &AnyResource{Receiver}) {
        BaloonToken.totalSupply = BaloonToken.totalSupply + amount
    ...
    init() {
        ...
        self.account.save(<-create VaultMinter(), to: /storage/BaloonMinter)
        ...
        self.account.link<&VaultMinter>(/private/Minter, target: /storage/BaloonMinter)
    }
}
